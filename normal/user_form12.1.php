   ...
   <div class="row-fluid">
                        ...
                        <div class="block">
                            <div class="navbar navbar-inner block-header">
                                <div class="muted pull-left">Ready File Record</div>
                            </div>
...
mysqli_query($conn,"insert into activity_log (date,username,action) values(NOW(),'$user_name','Ready File issue to Sadam')")or die(mysqli_error());
mysqli_query($conn,"UPDATE `transtbl` SET `trns_stu`='File Issued to Abid',dat='$dat',user='$session_id',trns_stj='7',trns_accounts='$trns_res' where tin_number = '$tin_number' and trns_stj='6'")or die(mysqli_error());
...
mysqli_query($conn,"INSERT INTO `borrow` (`employee_number`, `tin_number`, `date_borrow`, `ti_barrow`, `file_status`, `date_return`, `ti_return`, `emp_request`, `cmnt`, reason, stj) VALUES ('20', '$tin_number', '$dat', '$tim', 'borrowed', '0', '0', 'Abid', '$trns_res', 'Ready File', '1')")or die(mysqli_error());
mysqli_query($conn,"update file set trade_name = '20',status='borrowed' ,stj='7' where tin_number = '$tin_number' ")or die(mysqli_error());
...
?>
 ...
 <?php  $from1=0;  
			$user_query2 = mysqli_query($conn,"SELECT * FROM `staff` WHERE `staff_id`='17'")or die(mysqli_error());
			 while($row2 = mysqli_fetch_array($user_query2)){
			 $from1= $row2['email'];} 
			 
			$user_query3 = mysqli_query($conn,"select * from staff where staff_id='20'")or die(mysqli_error());
    ...
    $from = "$from1";
    $to = "$from2";
    $subject = "Ready File Issue Sadam ";
    $message = "Status send";
    $headers = "From:" . $from;
    if(mail($to,$subject,$message, $headers)) {
     ...
     ?>
<script>
  window.location = "edit_user12.1.php"; 
</script>
<?php
}  
//Mail
