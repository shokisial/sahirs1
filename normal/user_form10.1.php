...
mysqli_query($conn,"insert into activity_log (date,username,action) values(NOW(),'$user_name','Ready File issue to C.E')")or die(mysqli_error());
mysqli_query($conn,"UPDATE `transtbl` SET `trns_stu`='File Issued to C.E',dat='$dat',user='$session_id',trns_stj='5',trns_accounts='$trns_res' where tin_number = '$tin_number' and trns_stj='4'")or die(mysqli_error());
...
mysqli_query($conn,"INSERT INTO `borrow` (`employee_number`, `tin_number`, `date_borrow`, `ti_barrow`, `file_status`, `date_return`, `ti_return`, `emp_request`, `cmnt`,stj) VALUES ('1', '$tin_number', '$dat', '$tim', 'borrowed', '0', '0', 'Urooj', '$trns_res','1')")or die(mysqli_error());
mysqli_query($conn,"update file set trade_name = '1',status='borrowed' ,stj='5' where tin_number = '$tin_number' ")or die(mysqli_error());
...
?>
 ...
 <?php  $from1=0;  
			$user_query2 = mysqli_query($conn,"SELECT * FROM `staff` WHERE `staff_id`='21'")or die(mysqli_error());
			 while($row2 = mysqli_fetch_array($user_query2)){
			 $from1= $row2['email'];} ?>
<?php  $from2=0;  
			$user_query3 = mysqli_query($conn,"select * from staff where staff_id='1'")or die(mysqli_error());
...
<?php
    ...
    if(mail($to,$subject,$message, $headers)) {
     ...
     ?>
<script>
  window.location = "edit_user10.1.php"; 
</script>
<?php
}  
//Mail
