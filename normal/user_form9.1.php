   ...
   <div class="row-fluid">
                        ...
                        <div class="block">
                            <div class="navbar navbar-inner block-header">
                                <div class="muted pull-left">Accounts Ready File Clearence</div>
                            </div>
...
$query11 = mysqli_query($conn,"SELECT * FROM `ndc` WHERE `tin_number`='$tin_number' and `ndc_stj`='4' ")or die(mysqli_error());
								$row11 = mysqli_fetch_array($query11);
{ $brname=$row11['buyername']; $frname=$row11['fathername'];  
  $adr=$row11['address']; $cnc=$row11['cnic']; $cnt=$row11['cntno'];
  
  $jbrname=$row11['jbuyername']; $jfrname=$row11['jfathername'];  
  $jadr=$row11['jaddress']; $jcnc=$row11['jcnic']; $jcnt=$row11['jcntno'];$tp=$row11['type'];
}
	
								
mysqli_query($conn,"insert into activity_log (date,username,action) values(NOW(),'$user_name','Ready File issue to Urooj')")or die(mysqli_error());

mysqli_query($conn,"UPDATE `file` SET `file_name`='$brname',`fathername`='$frname',`cnicno`='$cnc',`address`='$adr',`cnt_no`='$cnt',`jfile_name`='$jbrname',`jfathername`='$jfrname',`jcnicno`='$jcnc',`jaddress`='$jadr',`jcnt_no`='$jcnt',`file_type`='$tp' where tin_number = '$tin_number'")or die(mysqli_error());


mysqli_query($conn,"UPDATE `transtbl` SET `trns_stu`='File Issued to Urooj',dat='$dat',user='$session_id',trns_stj='4',trns_accounts='$trns_res' where tin_number = '$tin_number' and trns_stj='3'")or die(mysqli_error());
...
mysqli_query($conn,"INSERT INTO `borrow` (`employee_number`, `tin_number`, `date_borrow`, `ti_barrow`, `file_status`, `date_return`, `ti_return`, `emp_request`, `cmnt`, reason, stj) VALUES ('21', '$tin_number', '$dat', '$tim', 'borrowed', '0', '0', 'Urooj', '$trns_res', 'Pending Ready', '1')")or die(mysqli_error());
...
<?php
    ...
    if(mail($to,$subject,$message, $headers)) {
     ...
     ?>
<script>
  window.location = "edit_user9.1.php"; 
</script>
<?php
}  
//Mail
